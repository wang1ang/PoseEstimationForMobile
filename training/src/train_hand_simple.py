...
from dataset_hand import get_dataset
from networks import get_network
from dataset_augment import set_network_input_wh, set_network_scale

def get_input_iter(batchsize, epoch, is_train=True):
    if is_train is True:
        input_pipeline = get_dataset(batch_size=batchsize, epoch=epoch, buffer_size=100, is_train=True)
    else:
        input_pipeline = get_dataset(batch_size=batchsize, epoch=epoch, buffer_size=100, is_train=False)
    iter = input_pipeline.make_one_shot_iterator()
    return iter

def get_loss_and_output(model, batchsize, input_image, input_heat, input_mask, reuse_variables=None):
    losses = []
    ...
    for idx, pred_heat in enumerate(pred_heatmaps_all):
        ...
        loss_l2 = tf.reduce_sum(tf.square(pred_heat - input_heat), [1,2]) * input_mask / 2
        #loss_l2 = tf.nn.sigmoid_cross_entropy_with_logits(logits=tf.concat(pred_heat, axis=0), labels=input_heat)
        losses.append(loss_l2)
    tf_regularization_losses = tf.losses.get_regularization_losses()
    #slim_regularization_losses = slim.losses.get_regularization_losses()

    total_loss = tf.reduce_sum(losses) / batchsize + tf.add_n(tf_regularization_losses)
...
def main(argv=None):
    ...
    config_file = "experiments/mv2_cpm_hand.cfg" #"experiments/mv2_cpm.cfg"
    if len(argv) != 1:
        config_file = argv[1]
    ...
    dataset_hand.set_config(params)
    ...
    with tf.Graph().as_default(), tf.device("/cpu:0"):
        train_dataset = get_dataset(params['batchsize'], params['max_epoch'], buffer_size=100, is_train=True)
        ...
        if platform.system() == 'Darwin':
            ...
            with tf.device("/cpu:0"):
                with tf.name_scope("CPU_0"):
                    input_image, input_heat, input_mask = train_iterator.get_next() #input_iterator.get_next()
                    loss, last_heat_loss, pred_heat = get_loss_and_output(params['model'], params['batchsize'], input_image, input_heat, input_mask, reuse_variable)
                    reuse_variable = True
